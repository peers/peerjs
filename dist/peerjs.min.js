...
"use strict";exports.__esModule=!0;var e=require("./util"),t=require("./adapter");exports.Negotiator={pcs:{data:{},media:{}},queue:[]},exports.Negotiator._idPrefix="pc_",exports.Negotiator.startConnection=function(t,o){var i=exports.Negotiator._getPeerConnection(t,o);if(t.pc=t.peerConnection=i,"media"===t.type&&o._stream&&i.addStream(o._stream),o.originator){if("data"===t.type){var r={};e.util.supports.sctp||(r={reliable:o.reliable});var n=i.createDataChannel(t.label,r);t.initialize(n)}exports.Negotiator._makeOffer(t)}else exports.Negotiator.handleSDP("OFFER",t,o.sdp)},exports.Negotiator._getPeerConnection=function(t,o){exports.Negotiator.pcs[t.type]||e.util.error(t.type+" is not a valid connection type. Maybe you overrode the `type` property somewhere."),exports.Negotiator.pcs[t.type][t.peer]||(exports.Negotiator.pcs[t.type][t.peer]={});var i;exports.Negotiator.pcs[t.type][t.peer];return o.pc&&(i=exports.Negotiator.pcs[t.type][t.peer][o.pc]),i&&"stable"===i.signalingState||(i=exports.Negotiator._startPeerConnection(t)),i},exports.Negotiator._startPeerConnection=function(o){e.util.log("Creating RTCPeerConnection.");var i=exports.Negotiator._idPrefix+e.util.randomToken(),r={};"data"!==o.type||e.util.supports.sctp?"media"===o.type&&(r={optional:[{DtlsSrtpKeyAgreement:!0}]}):r={optional:[{RtpDataChannels:!0}]};var n=new t.RTCPeerConnection(o.provider.options.config,r);return exports.Negotiator.pcs[o.type][o.peer][i]=n,exports.Negotiator._setupListeners(o,n,i),n},exports.Negotiator._setupListeners=function(t,o,i){var r=t.peer,n=t.id,a=t.provider;e.util.log("Listening for ICE candidates."),o.onicecandidate=function(o){o.candidate&&(e.util.log("Received ICE candidates for:",t.peer),a.socket.send({type:"CANDIDATE",payload:{candidate:o.candidate,type:t.type,connectionId:t.id},dst:r}))},o.oniceconnectionstatechange=function(){switch(o.iceConnectionState){case"failed":e.util.log("iceConnectionState is disconnected, closing connections to "+r),t.emit("error",new Error("Negotiation of connection to "+r+" failed.")),t.close();break;case"disconnected":e.util.log("iceConnectionState is disconnected, closing connections to "+r);break;case"completed":o.onicecandidate=e.util.noop}},o.onicechange=o.oniceconnectionstatechange,e.util.log("Listening for data channel"),o.ondatachannel=function(t){e.util.log("Received data channel");var o=t.channel;a.getConnection(r,n).initialize(o)},e.util.log("Listening for remote stream"),o.ontrack=function(t){e.util.log("Received remote stream");var o=t.streams[0],i=a.getConnection(r,n);"media"===i.type&&i.addStream(o)}},exports.Negotiator.cleanup=function(t){e.util.log("Cleaning up PeerConnection to "+t.peer);var o=t.pc;o&&(o.readyState&&"closed"!==o.readyState||"closed"!==o.signalingState)&&(o.close(),t.pc=null)},exports.Negotiator._makeOffer=function(t){var o=t.pc;o.createOffer(function(i){e.util.log("Created offer."),!e.util.supports.sctp&&"data"===t.type&&t.reliable&&(i.sdp=Reliable.higherBandwidthSDP(i.sdp)),o.setLocalDescription(i,function(){e.util.log("Set localDescription: offer","for:",t.peer),t.provider.socket.send({type:"OFFER",payload:{sdp:i,type:t.type,label:t.label,connectionId:t.id,reliable:t.reliable,serialization:t.serialization,metadata:t.metadata,browser:e.util.browser},dst:t.peer})},function(o){"OperationError: Failed to set local offer sdp: Called in wrong state: kHaveRemoteOffer"!=o&&(t.provider.emitError("webrtc",o),e.util.log("Failed to setLocalDescription, ",o))})},function(o){t.provider.emitError("webrtc",o),e.util.log("Failed to createOffer, ",o)},t.options.constraints)},exports.Negotiator._makeAnswer=function(t){var o=t.pc;o.createAnswer(function(i){e.util.log("Created answer."),!e.util.supports.sctp&&"data"===t.type&&t.reliable&&(i.sdp=Reliable.higherBandwidthSDP(i.sdp)),o.setLocalDescription(i,function(){e.util.log("Set localDescription: answer","for:",t.peer),t.provider.socket.send({type:"ANSWER",payload:{sdp:i,type:t.type,connectionId:t.id,browser:e.util.browser},dst:t.peer})},function(o){t.provider.emitError("webrtc",o),e.util.log("Failed to setLocalDescription, ",o)})},function(o){t.provider.emitError("webrtc",o),e.util.log("Failed to create answer, ",o)})},exports.Negotiator.handleSDP=function(o,i,r){r=new t.RTCSessionDescription(r);var n=i.pc;e.util.log("Setting remote description",r),n.setRemoteDescription(r,function(){e.util.log("Set remoteDescription:",o,"for:",i.peer),"OFFER"===o&&exports.Negotiator._makeAnswer(i)},function(t){i.provider.emitError("webrtc",t),e.util.log("Failed to setRemoteDescription, ",t)})},exports.Negotiator.handleCandidate=function(o,i){var r=i.candidate,n=i.sdpMLineIndex;o.pc.addIceCandidate(new t.RTCIceCandidate({sdpMLineIndex:n,candidate:r})),e.util.log("Added ICE candidate for:",o.peer)};
